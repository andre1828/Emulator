class CPU
  ...
  def execute_add fst_parameter, snd_parameter
    if (is_register? fst_parameter) && (is_register? snd_parameter)
      fst_register_value = self.instance_variable_get("@#{fst_parameter}")
      snd_register_value = self.instance_variable_get("@#{snd_parameter}")

      self.instance_variable_set("@#{fst_parameter}", fst_register_value + snd_register_value)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")}"
      ...
      register_value = self.instance_variable_get("@#{fst_parameter}")

      self.instance_variable_set("@#{fst_parameter}", @value + register_value)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
    elsif (is_register? fst_parameter) && (snd_parameter.kind_of? Integer)
      register_value = self.instance_variable_get("@#{fst_parameter}")
      self.instance_variable_set("@#{fst_parameter}", register_value + snd_parameter)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
      ...
      register_value = self.instance_variable_get("@#{snd_parameter}")

      @bus.send_ram [@write_op, fst_mem_address + 4, fst_address_value + register_value]
      @ram.receive_ram
    elsif (is_mem_address? fst_parameter) && (snd_parameter.kind_of? Integer)
      fst_mem_address = mem_address_2_decimal fst_parameter
  ...
  def execute_mov fst_parameter, snd_parameter
    if (is_register? fst_parameter) && (is_register? snd_parameter) 
      snd_register_value = self.instance_variable_get("@#{snd_parameter}")
      self.instance_variable_set("@#{fst_parameter}", snd_register_value)
      puts "@#{fst_parameter} is now #{self.instance_variable_get("@#{fst_parameter}")}"
      ...
      self.instance_variable_set("@#{fst_parameter}", @value)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
    elsif (is_register? fst_parameter) && (snd_parameter.kind_of? Integer)
      self.instance_variable_set("@#{fst_parameter}", snd_parameter)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
    elsif (is_mem_address? fst_parameter) && (is_mem_address? snd_parameter)
      # fst_address_value
      fst_mem_address = mem_address_2_decimal fst_parameter
      ...
      @bus.send_ram [@write_op, mem_address + 4, self.instance_variable_get("@#{snd_parameter}")]
      @ram.receive_ram
    elsif (is_mem_address? fst_parameter) && (snd_parameter.kind_of? Integer)
      mem_address = mem_address_2_decimal fst_parameter
  ...
  def execute_imul fst_parameter, snd_parameter, trd_parameter
    if (is_register? fst_parameter) && (is_register? snd_parameter) && (is_register? trd_parameter)
       fst_register_value = self.instance_variable_get("@#{fst_parameter}")
      snd_register_value = self.instance_variable_get("@#{snd_parameter}")
      trd_register_value = self.instance_variable_get("@#{trd_parameter}")

      result = fst_register_value * snd_register_value * trd_register_value

      self.instance_variable_set("@#{fst_parameter}", result )
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
      ...
      self.instance_variable_set("@#{fst_parameter}", result)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
    elsif (is_register? fst_parameter) && (is_register? snd_parameter) && (trd_parameter.kind_of? Integer)
      fst_register_value = self.instance_variable_get("@#{fst_parameter}")
      snd_register_value = self.instance_variable_get("@#{snd_parameter}")
      ...
      fst_register_value = self.instance_variable_get("@#{fst_parameter}")
      snd_register_value = self.instance_variable_get("@#{trd_parameter}")
      ...
      self.instance_variable_set("@#{fst_parameter}", result)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
    elsif (is_register? fst_parameter) && (is_mem_address? snd_parameter) && (trd_parameter.kind_of? Integer)
      fst_register_value = self.instance_variable_get("@#{fst_parameter}")
      ...
      self.instance_variable_set("@#{fst_parameter}", result)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
    elsif (is_register? fst_parameter) && (snd_parameter.kind_of? Integer) && (trd_parameter.kind_of? Integer)
      register_value = self.instance_variable_get("@#{fst_parameter}")
      ...
      self.instance_variable_set("@#{fst_parameter}", result)
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
    elsif (is_register? fst_parameter) && (snd_parameter.kind_of? Integer) && (is_register? trd_parameter)
      fst_register_value = self.instance_variable_get("@#{fst_parameter}")
      snd_register_value = self.instance_variable_get("@#{trd_parameter}")
      ...
      puts "register #{fst_parameter} now holds the value #{self.instance_variable_get("@#{fst_parameter}")} "
    elsif (is_register? fst_parameter) && (snd_parameter.kind_of? Integer) && (is_mem_address? trd_parameter)
      register_value = self.instance_variable_get("@#{fst_parameter}")
      mem_address = mem_address_2_decimal fst_parameter
      ...
      @ram.receive_ram
    elsif (is_mem_address? fst_parameter) && (snd_parameter.kind_of? Integer) && (trd_parameter.kind_of? Integer)
      mem_address = mem_address_2_decimal fst_parameter
      ...
      @bus.send_ram [@write_op, mem_address + 4, result]
      @ram.receive_ram
    elsif (is_mem_address? fst_parameter) && (snd_parameter.kind_of? Integer) && (is_register? trd_parameter)
      mem_address = mem_address_2_decimal fst_parameter
      register_value = self.instance_variable_get("@#{trd_parameter}")
      ...
      @bus.send_ram [@write_op, mem_address + 4, result]
      @ram.receive_ram
    elsif (is_mem_address? fst_parameter) && (snd_parameter.kind_of? Integer) && (is_mem_address? trd_parameter)
      fst_mem_address = mem_address_2_decimal fst_parameter
      snd_mem_address = mem_address_2_decimal trd_parameter
      ...
      @bus.send_ram [@write_op, fst_mem_address + 4, result]
      @ram.receive_ram
    elsif (is_mem_address? fst_parameter) && (is_mem_address? snd_parameter) && (trd_parameter.kind_of? Integer)
      fst_mem_address = mem_address_2_decimal fst_parameter
      snd_mem_address = mem_address_2_decimal snd_parameter
      ...
      mem_address = mem_address_2_decimal fst_parameter
      fst_register_value = self.instance_variable_get("@#{snd_parameter}")
      snd_register_value = self.instance_variable_get("@#{trd_parameter}")
      ...
      @bus.send_ram [@write_op, fst_mem_address + 4, result]
      @ram.receive_ram
    elsif (is_mem_address? fst_parameter) && (is_register? snd_parameter) && (trd_parameter.kind_of? Integer)
      mem_address = mem_address_2_decimal fst_parameter
      register_value = self.instance_variable_get("@#{snd_parameter}")
  ...
  def is_mem_address? parameter
    return false if !parameter.kind_of? String
    parameter.start_with? "0X"
  end

  def is_instruction? signal
    signal.kind_of?(Array)
  end
end
